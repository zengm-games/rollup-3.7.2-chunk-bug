import { c as GAME_NAME } from './getCols-55f556d5.js';
import { U as logEvent } from './index-fcb3f17e.js';
...
if ("serviceWorker" in navigator) {
  ...
  const showUpdateAvailableNotification = () => {
    if (!updateAvailableNotificationShowing) {
      window._wb_updateAndRefresh = button => {
  ...
  wb.addEventListener("controlling", event => {
    if (event.isUpdate || event.isExternal || updateAvailableNotificationShowing) {
      window.location.reload();
    }
  });
  wb.register();
