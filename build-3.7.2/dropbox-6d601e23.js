import { f as bySport } from './getCols-55f556d5.js';
...
var DropboxResponseError = /*#__PURE__*/function (_Error) {
  ...
  function DropboxResponseError(status, headers, error) {
    var _this;
    _classCallCheck$3(this, DropboxResponseError);
...
var DropboxResponse = function DropboxResponse(status, headers, result) {
  _classCallCheck$2(this, DropboxResponse);
  this.status = status;
...
var DropboxAuth = /*#__PURE__*/function () {
  function DropboxAuth(options) {
    _classCallCheck$1(this, DropboxAuth);
    options = options || {};
    if (isBrowserEnv()) {
...
const getAuthenticationUrl = async lid => {
...
const handleStreamError = async (stream, error, lid) => {
  if (error.status === 401) {
  ...
  let errorMessage;
  if (error.status === 409 && error.error?.error_summary?.includes("insufficient_space")) {
    errorMessage = "Your Dropbox is full. Either delete some files from Dropbox or upgrade your Dropbox account.";
...
const dropboxStream = async ({
  ...
  const buffer = new Buffer$1(dropbox, lid);
      ...
      try {
        ...
        try {
          ...
          if (error.status === 409) {
            ...
            fileURL = error?.error?.error?.shared_link_already_exists?.metadata?.url;
          }
...
export { dropboxStream, getAuthenticationUrl };
